...
# Check if root .env.production exists
  ...
  echo "❌ No .env.local or .env.production file found in project root!"
  echo "   Please create $PROJECT_ROOT/.env.local or $PROJECT_ROOT/.env.production with required API keys"
  exit 1
fi
